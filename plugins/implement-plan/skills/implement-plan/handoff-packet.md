...
>
> Do not add test infrastructure for this task alone. Test the behavior this slice changes; do not backfill unrelated coverage.
>
> **A test must be able to disagree with the code, and only about behavior.** Write every expected value down as a literal you worked out from the plan on a small input. Never produce it with the code's own formula, a production helper, or an imported constant: both sides then move together and the test passes whatever the code does. This holds when the plan says to reuse existing wording or logic — reuse it in the code, and state the expected result in the test as a literal. Assert what the plan fixes and a caller can observe: results, errors, effects on collaborators, the facts a message must carry. Do not assert which helper ran, call order the plan does not require, a constant against its own value, every word of an output when the plan fixes only some of it, or a repo sample file someone will edit. Before keeping an assertion, name a rewrite with the same behavior that would turn it red; if you can, loosen it.
>
> **A bug fix does not automatically earn a new test.** First find what let the bug through. It is one of two things:
>
> - **An existing assertion already exercises that input and expects the wrong value.** Correct that assertion in place. It is your failing test: run it, watch it fail, fix the code. Add no second test on the same input; that is a duplicate. If the correction makes the test's name or description untrue of what it now checks, rename or reword it in the same edit; never move the assertion elsewhere to keep the old name.
...
>
> Then, for each behavior this slice changes, write a **test map** entry under **Proof**, in exactly this shape:
>
> ```
...
> ```
>
> The steps are the **setup calls and actions only** — every fixture, helper, or command call the test makes before and while exercising the code. Assertions are not steps: they are what is new by definition, and they never make a test new. The nearest test is the existing test that calls the same command, function, or endpoint the most; `nearest: none` is allowed only when no test in the repository calls that command or function at all — a test that sets up differently, or asserts on something no test has asserted on yet, is still the nearest test. Mark each step with the nearest test's name when it already performs that call, whatever its inputs. When the new test builds state by hand (writing a raw file, inserting a row) where the existing test used a helper, count that as one step and leave it unmarked; the helper calls before and after it are still shared, so mark them. `s` is the marked steps, `n` the steps listed, and `p` is `s / n` written as a percentage, rounded down: 5 of 6 is 83%.
>
> A filled-in entry, for a flag added to an `add` command whose table test already covers `add`:
>
> ```
...
> ```
>
> When the plan's acceptance criterion already names the test that proves it (`— extends <file> <TestName>`), that test is the answer: add the case there and skip the count. Count only for behaviors whose criterion says `new test` or names no test.
>
> **At 60% or more, extend**: a row in that test's table, a step or branch in its workflow, and the new assertion placed after the step it checks (a table without a column for it gets the column; a workflow test gets the read-the-file check after the call that wrote it). The share is the decision; at 60% or more there is no judgment call left to make. "Technically qualifies as extend, but it needed distinct assertions", "a different item shape", "its own function for isolation", and "a judgment call" are the sentences that precede a duplicate — when one appears in your reasoning, add the case to the nearest test. Under 60%, write a new test and keep the list in the map. When the only shared step is the call to the function under test — a pure function tested one call per case — the test's name is the one thing that separates its cases, so the case goes into the named grouping the language's usual test framework offers (a table row, a subtest, a nested `describe`/`it`, a parametrized case), named for the behavior, with the setup kept once above it; when the framework has no such grouping, a behavior-named test. A test extended under TDD still fails first: add the case, watch it fail, then write the code. A new test at 60% or more, a new test with no listed steps, or a `nearest: none` for a command some test already calls, will be sent back with the recount attached.
>
...
> **Integration and E2E tests are user journeys, and one journey owns each workflow.** Design each test the way a real consumer of this project behaves: run the real CLI binary, call the real HTTP endpoint, drive the real UI flow, read the real file the user would read. A workflow an existing journey already walks gets its new step or branch in that journey; a new journey is for a workflow no journey walks yet. Count it the same way as a unit test: a journey whose setup and action steps are 60% or more already performed by an existing journey is that journey, extended. **Mock only what cannot run for real** — third-party services, the wall clock, external networks, paid APIs. Everything else executes from the real codebase. Follow this repository's existing layout, runner, fixtures, and naming exactly: `<describe>`.

Add when the user asked for real dependencies:

> **Real dependencies, not fakes.** For databases, queues, caches, object stores, and similar, use testcontainers (or this repository's existing equivalent) to run a real instance in the test. Do not stub the driver.
...
> - Every document that describes the changed behavior is updated in this   diff, or the evidence names the search that found none
...
> - Nothing in the diff or the commit message carries a label from this run:   no gate ID (G1–G4), slice or wave name, round or pass number, finding   number, step number, or ticket title. The plan that gives those meaning is   not in the repository, so a reader of the code would find a pointer to   nothing. Write what the code does, or write nothing.
...
- Each plan item, quoted, with the file:line that fulfils it and the test that proves it — or "not done" with the reason.
