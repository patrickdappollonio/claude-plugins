...
The brief has one legitimate job and one illegitimate one, and the difference is what this panel lives or dies on. **Legitimate:** it bounds scope — "don't report the absence of work nobody agreed to do." **Illegitimate:** it defines correctness — "the code does this because the plan says so, therefore it isn't a finding."

The second reading makes the brief unfalsifiable. A reviewer finds something wrong, checks the brief, sees the behaviour was agreed, and files it under *out of scope* instead of under *bug*. That suppression is silent, and because it is deterministic it fires again on every later round — which is how a defect survives review after review while everybody works honestly.

**Agreement moves the fault from the implementer to the design. It does not make the fault disappear.** Whether the change matches what was approved, and whether what was approved was right, are two separate questions, and this panel answers both: the *Spec Conformance Auditor* owns the first, the *Premise Auditor* the second, and the scope rule tells every other reviewer that "it was in the plan" is never a reason to withhold a defect.
...
**One exception to "all 18": the user chose a subset.** When the user picked specific reviewers, run exactly that subset. Everything else about this skill still applies unchanged: the brief, the scope rule, the verifier, the validator, and the report. Treat the reviewers left out as user-decided skips and name them in the report's close, so a clean report is never read as a clean bill of health.
...
Collect all findings from all reviewers. Then dispatch **one separate verifier subagent** (the *False-Positive Filter*, charter below) — also `model: "sonnet"` in Claude Code. Give it the full list of findings plus the brief — including the approved design artifact itself, when one exists, since it cannot check a conformance finding without it — the diff, and the changed files. It re-checks each finding against the actual code and returns a verdict: **confirmed / not-confirmed**, with a one-line reason. This agent must be fresh and standalone so it does not inherit any reviewer's enthusiasm.
...
You hold code-level material: findings full of symbols, drafted fixes, validator notes. **The report is not that material — it is a plain re-telling of it, written for someone who never saw the code and never will.** Eighteen agents just read their way across the codebase; the user did none of that and on most surfaces could not have, so every identifier left in the prose moves the work of understanding onto the one person who cannot do it.

**The report is exactly these parts, in this order:**

1. **A TL;DR of 2–4 sentences** — what you reviewed, whether the change matches what was agreed, whether what was agreed turned out to be right, how many real problems survived verification, and whether any are serious. Write it plainly without labelling it as plain.
...
3. **One line on whether the design held up** — separate from the line above, because the answers are independent: a change can match the plan exactly and still be wrong, and that is the case the user has least chance of catching alone. A clean conformance line never stands in for this one.
4. **A one-line count** — "9 confirmed issues across 5 files; 3 serious, 4 moderate, 2 minor."
5. **The findings, grouped** under short plain themes rather than charter names — *the plan itself has a problem*, *doesn't match what was agreed*, *could crash or break*, *security and access gaps*, *data ending up wrong*, *slow under heavy use*, *will confuse the next person*, *weak tests*, *claims that aren't true*.
6. **A close** — which reviewers were skipped and why, then anything the brief had declared out of scope that a reviewer flagged anyway, listed separately as *deliberately left out — flagged anyway*, one line each. Hold deep technical detail until asked.

**Two groups lead, in this order, whenever they have anything in them:**

1. ***The plan itself has a problem*** — the `design_is_wrong` findings. This outranks everything, because every group below it is measured against a plan this group says was wrong. Open it by saying the code does what was agreed and that is the problem, and make clear that fixing it means changing what was agreed — so nobody reads it as the implementer having slipped.
2. ***Doesn't match what was agreed*** — the conformance findings. Describe each gap as the user would experience it ("the list has no column showing which item is which, so the number shown is the count rather than the item's ID"), not as a diff against a document.
...
- **Where** — `path/file.go:142`. **This field is where every symbol lives** — function names, classes, variables, flags. It carries the code-level precision so the prose doesn't have to.
- **Severity** — serious / moderate / minor (the reviewers' high / medium / low).
- **Found by** — the reviewer, so the user can gauge the angle.

Write it in Simplified Technical English (ASD-STE100) or any equally plain register: short sentences, one idea each, active voice, present tense; common words; the effect first and the label last; what a person would observe rather than what the code does internally; a plain-words gloss on any term like "race condition" or "IDOR". Never sacrifice a real finding to be brief — plain is the goal, short is not. If the `effective-communicator` skill is installed, it governs the wording and this section defers to it.
...
This register governs the questions in step 8 too, and any explanation the user asks for along the way: "go deeper" means more of the reasoning, the sequence, and the consequence — not a switch into code-speak. If the user quotes symbols back at you they are reading the code alongside you, so match them for that exchange and reset to plain for the next one.

### 8. STOP — put each finding to the user, one at a time; do not change anything

**Reviewing and proposing fixes is the whole job. A described, validated fix is NOT permission to apply it.** The moment the report is delivered you stop and put the next move in the user's hands. Do not edit code, do not open files to "just apply the quick one," do not start drafting patches in the working tree. When no finding survived verification there is nothing to ask; the report is the end.
...
**Answers in their own words.** "Apply all of them", "just do everything", "the rest too" answer every remaining ordinary question at once: record it and stop asking those. Say "recorded as apply", never "applying now" — nothing is applied until the last question is answered, and the message must not claim otherwise. They never answer a `design_is_wrong` question, which is asked one by one regardless: "all of them" is an answer about the fixes they were offered, not approval of a design they have not been shown. Only a direct answer closes a question: a hedged or vague reply ("sure, whatever you think", "ok" to several alternatives) or a question back keeps it open — answer what they asked and put the same *Your call* again. Silence answers nothing: an unanswered finding stays undecided and nothing is applied, and every message says how many are open.
...
| "The fix is already written and validated, so applying it is one more step." | The fix being ready is exactly why you pause — so they can still apply, defer, or dismiss it. |
| "Eight findings, eight messages — I'll list them all in one and let them answer in bulk." | One question per message is the shape. The report already shows them all; the chat is where they decide one at a time without facing a wall of text. |
...
```json
{
  ...
  "evidence": "the specific code / interleaving / input that proves it — secret values redacted, never quoted",
...
- **Writing the report as anything other than the six parts in step 7.** The parts, their order, and the finding fields are the contract. Dropping the TL;DR, padding a finding past three sentences, labelling the summary as "plain terms," or letting symbol names into the prose all produce something that is not this report. Symbols have one home: the `Where` field.
...
- **Applying fixes before the user chooses.** This skill produces a *review with proposed fixes*, not changes to the code. A confirmed finding with a validated fix — even an obvious one-liner — does not authorize editing. Stop at the report and put each finding to the user in the `question-format.md` shape, one per message, until every one is decided.
- **Asking a finding through `AskUserQuestion`, several at once, or without the five parts.** The question tool holds one line per option and no flow; a menu of "explain / apply / triage" asks the user to decide about findings they cannot read away from the code. Every finding is one plain-text question — heading, situation, flow, fix, cost, your call — one per message. If you have not read `question-format.md` this session, you have skipped a step.
