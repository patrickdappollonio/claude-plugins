...
**One exception to "all 18": the user chose a subset.** When this skill was entered as an escalation from `adversarial-review-quick` — or the user otherwise picked specific reviewers — run exactly that subset (the quick panel's eight plus their picks, in the escalation case). Everything else about this skill still applies unchanged: the brief, the scope rule, the verifier, the validator, and the report. Treat the reviewers left out as user-decided skips and name them in the report's close, so a clean report is never read as a clean bill of health.
